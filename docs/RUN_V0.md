...
There is therefore no intended encrypted development database on which to safely apply either migration. The owner has subsequently authorized [the fixed human-only LUKS2 initialization procedure](STORAGE_INITIALIZATION.md), now the next operator action before Alpaca credential entry. The database/execution processes remain unprovisioned. **Do not run historical qualification blocks as bootstrap or send any secret in chat.** Preparing the approved storage procedure neither generates a TDE key in ordinary storage nor initializes OpenBao or resurrects --io-test.
